        ...
        def catch_all
          ...
          if lead = Lead.find_by_email(params['data']['email'])
            LeadScoringRule.find_all_by_event(params['event']).each do |rule|
              ...
              count = LeadScoringRuleCount.find_by_lead_and_rule(lead, rule) ||
                      LeadScoringRuleCount.new :lead => lead, :rule => rule
              ...
              unless rule.once && count.count > 0
                ...
                lead.versions.create "Score incremented by #{rule.points} points, new score: #{lead.score}"
